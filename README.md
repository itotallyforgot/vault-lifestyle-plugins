...
- **Output convention.** Every ingest plug-in writes `raw/<YYYY-MM-DD>-<slug>.md` with frontmatter compatible with second-brain's `/vault ingest` skill (`ingested: false`, `clipped_at`, source URL field, etc.). The shared `lib/vault_write/` codifies the contract.
