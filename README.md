...
- **Output convention.** Every ingest plug-in writes `raw/<slug>.md` with frontmatter compatible with markdown-vault's `/vault ingest` skill (`source_url`, `clipped_at`, `ingested: false` are required; rich optional fields preserved as source-page metadata). Slug shape is per-plug-in (e.g. YouTube uses `<video_id>-<sanitized-title>`). The shared `lib/frontmatter_schema.py` Pydantic model + `lib/schemas/raw_frontmatter.json` JSON Schema codify the contract.
