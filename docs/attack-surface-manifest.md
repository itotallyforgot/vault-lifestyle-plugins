...
| Trust boundary | The user-supplied `url` is untrusted text. `urlparse` rejects non-http(s) schemes (`_looks_like_url`) before yt-dlp sees it, and `cli._ingest_url` now also runs `inputs._parse_youtube_input` so the YouTube host allow-list is enforced on this single-URL path as well as the batch path (was batch-only — fixed, L3). The VTT body and metadata title flow into the slug (`slug.make`, which strips to `[a-z0-9-]`) and into the frontmatter (Pydantic-validated). Transcript text flows verbatim into the body of the raw markdown. |
...
| Trust boundary | The url-list file is treated as untrusted text. Each line is reparsed through `inputs._parse_youtube_input`, which enforces an allow-list of hosts (`youtube.com`, `m.youtube.com`, `music.youtube.com`, `youtu.be`). Handoff JSONL is shape-validated by `handoff._validate_shape` against a small allow-list of fields; unknown keys are rejected, and every record `url` is re-validated through `inputs._parse_youtube_input` then normalized to its canonical `youtu.be/<id>` form (host-allowlist bypass fixed, L3). `--run-id` is used as a path segment under `.vault-lifestyle/`; it is now validated against `^[A-Za-z0-9._-]+$` (rejecting separators and `..`) in `manifest.validate_run_id`, called from both CLI parse and `default_manifest_path` (path-traversal fixed, L3). |
...
| Misuse / abuse cases | — `--run-id "../../../etc/passwd"` → directory traversal under `<vault>/.vault-lifestyle/`. **Mitigated (L3):** `manifest.validate_run_id` rejects any run-id outside `^[A-Za-z0-9._-]+$` (and the `.`/`..` tokens) at CLI parse and again in `default_manifest_path`, so the run-id can no longer escape the staging area.<br>— Handoff record with both a benign `video_id` and a hostile non-YouTube `url` → batch-ingested via that url (cli._ingest_url checked scheme only). **Mitigated (L3):** `handoff._parse_line` re-validates every url through `_parse_youtube_input` and stores only the canonical `youtu.be/<id>`; `cli._ingest_url` also applies the host check.<br>— Handoff JSONL with thousands of records → resource exhaustion: no record-count cap. **P2.**<br>— Handoff record with attacker-chosen `playlist_title` flowing into the raw frontmatter via `cli._raw_provenance`: the value is preserved verbatim into YAML. yaml.safe_dump escapes, Pydantic re-validates, so this is parse-safe. **P3.**<br>— `--url-file` pointed at a 100 MB random-bytes file: read with `Path.read_text(encoding="utf-8")` — utf-8 decode error rejects it. **P3.** |
...
| Trust boundary | Handoff records pass through `handoff._validate_shape` (unknown fields rejected, type-checked, `playlist_index >= 1`). Every record `url` is re-validated through `inputs._parse_youtube_input` — enforcing the YouTube host allow-list even when a `video_id` is also present — and normalized to its canonical `youtu.be/<id>` form before it can reach the ingest leg (L3). The `source_provider` field is preserved verbatim into the manifest and frontmatter. |
...
| Misuse / abuse cases | — `--run-id "../../etc/passwd"` → manifest write outside `.vault-lifestyle/`. Same vector as batch ingest. **Mitigated (L3):** `manifest.validate_run_id` rejects traversing run-ids at CLI parse and in `default_manifest_path`.<br>— `--evidence-url` set to a phishing URL → that URL ends up in the manifest and any downstream report. Treat manifest contents as user-provided; do not auto-fetch evidence URLs. **P3** for this tool; **P1** if a future report auto-resolves them. |
...
| Filesystem access | Reads: `~/.config/vault-lifestyle-plugins/spotify.toml` (CLIENT_ID config). Writes: the token cache. The cache path resolution chain is `$VAULT_SPOTIFY_TOKEN_CACHE` → `$XDG_DATA_HOME/vault-lifestyle-plugins/spotify-tokens.json` → `~/.local/share/vault-lifestyle-plugins/spotify-tokens.json`. The cache file contains the refresh token (long-lived) and the access token (short-lived). Parent dirs are created via `Path.mkdir(parents=True, exist_ok=True)`. **Tokens are stored with default umask** — neither this code nor spotipy applies `chmod 600`. See misuse case. |
